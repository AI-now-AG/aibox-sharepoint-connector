...
export const POST: APIRoute<CreatePromptParams> = async (ctx) => {
  ...
  const maxPositionPrompt = await GlobalPromptModel.getMaxPosition(_group);
  ...
  // Extract _id and the rest of the fields
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { _id, ...rest } = data;
  const prompt: Partial<Prompt> = {
    ...rest, // all fields except _id
  ...
  try {
    if (prompt) {
      await GlobalPromptModel.create(prompt);
...
export const PUT: APIRoute<CreatePromptParams> = async (ctx) => {
  ...
  try {
    if (prompt && data._id) {
      await GlobalPromptModel.update(data._id, prompt);
...
export const GET: APIRoute = async (ctx) => {
  try {
    ...
    if (promptId) {
      const prompt = await GlobalPromptModel.get(promptId);
      if (!prompt) {
        return new Response(
          JSON.stringify({
...
export const DELETE: APIRoute<PromptParams> = async (ctx) => {
  try {
    ...
    if (data._id) {
      const result = await GlobalPromptModel.remove(data._id.toString());
