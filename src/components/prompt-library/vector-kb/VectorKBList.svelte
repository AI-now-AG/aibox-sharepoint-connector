  ...
  function getStatusBadge(status: DataSourceStatus): { class: string; text: string } {
    switch (status) {
      case DataSourceStatus.Pending:
        return { class: "bg-warning/15 text-warning border-warning/30", text: t("vector-kb.status.pending") };
      case DataSourceStatus.Processing:
        return { class: "bg-info/15 text-info border-info/30", text: t("vector-kb.status.processing") };
      case DataSourceStatus.Completed:
        return { class: "bg-success/15 text-success border-success/30", text: t("vector-kb.status.completed") };
      case DataSourceStatus.Failed:
        return { class: "bg-error/15 text-error border-error/30", text: t("vector-kb.status.failed") };
      default:
            ...
            <tbody>
              {#each sortedDataSources as source}
                ...
                <tr class="{selectedFiles.has(source._id) ? 'bg-primary/10' : ''} hover:bg-base-200/50">
                  ...
                  <td>
                    <!-- 3-dot dropdown menu for screens < 1536px -->
                    <div class="dropdown dropdown-end 2xl:hidden">
                    ...
                    <!-- Individual buttons for screens >= 1536px -->
