<script lang="ts">
  ...
  import { type PromptCartItem } from "./PromptList.svelte";
  ...
  interface Props {
    ...
    onFilterCallback: Function;
  }

  let { items = [], categoryList = [], onFilterCallback }: Props = $props();

  let searchQuery = $state("");
  let filteredPrompts = $state(items);
  ...
  function groupItemChanged() {
    ...
    if (checkedCategories.length > 0 || checkedGroups.length > 0) {
      filteredPrompts = items.filter((prompt) => {
        return (
          prompt.tags?.some(
            (tag) =>
              checkedGroups.some(
    ...
    onFilterCallback(filteredPrompts);
  }
</script>
