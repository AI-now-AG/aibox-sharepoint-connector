...
export function formatCitations(
  ...
  const updatedString = inputString.replace(/\[(\d+)\]/g, (match, p1) => {
    ...
    if (index >= 0 && index < citations.length) {
      return `<a href='${citations[index]}' target='_blank' class='bg-gray-100 hover:bg-blue-600 text-blue-600 hover:text-white text-xs font-normal ml-1 rounded-sm justify-center items-center'>[${p1}]</a>`;
    }
    return match;
  });
